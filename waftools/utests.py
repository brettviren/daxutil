...
def utesting(bld, name, use=''):
    appsrc = bld.path.ant_glob('apps/*.c*')
    checksrc = bld.path.ant_glob('test/check_*.c*')
    testsrc = bld.path.ant_glob('test/test_*.c*')
    test_scripts = bld.path.ant_glob('test/test_*.sh') + bld.path.ant_glob('test/test_*.py')

    if getattr(bld.options, "notests", False):
        print ("no tests")
        return
    if not any([appsrc,checksrc,testsrc, test_scripts]):
        return
